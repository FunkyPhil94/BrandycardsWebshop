...
const products: Product[] = [
  { title: "2024 Topps Chrome UEFA Club Competitions Lamine Yamal RC #150", category: "Festpreis", price: "39,90 €", meta: "Demo · eBay synchronisiert · Sofort-Kaufen", accent: "gold", badge: "Demo-Angebot", description: "Topps Chrome UEFA Club Competitions · Rookie Card · Near Mint" },
  { title: "2023-24 Panini Prizm Premier League Bukayo Saka Silver #181", category: "Festpreis", price: "24,90 €", meta: "Demo · eBay synchronisiert · Sofort-Kaufen", accent: "red", badge: "Demo-Angebot", description: "Panini Prizm Premier League · Silver Prizm · Near Mint" },
  { title: "2022 Topps Finest Bundesliga Jamal Musiala Refractor #72", category: "Festpreis", price: "18,50 €", meta: "Demo · eBay synchronisiert · Sofort-Kaufen", accent: "blue", badge: "Demo-Angebot", description: "Topps Finest Bundesliga · Refractor · Near Mint" },
  { title: "2024 Panini Select FIFA Lamine Yamal Rookie /99", category: "Vormerkliste", meta: "Demo · Noch nicht im Verkauf", accent: "violet", badge: "Demo-Karte", description: "Sichere dir eine Nachricht, sobald diese Karte live geht." },
  { title: "2023-24 Topps Chrome Bundesliga Florian Wirtz Auto", category: "Auktion", meta: "Demo · Auktion auf eBay · endet bald", accent: "silver", badge: "eBay Auktion", description: "Nur als Hinweis im Webshop – der Kauf erfolgt direkt bei eBay." },
];
...
export default function Home() {
  ...
  const filteredProducts = useMemo(() => products.filter((product) => (activeFilter === "Alle Angebote" || product.category === activeFilter) && product.title.toLowerCase().includes(query.toLowerCase())), [activeFilter, query]);
  ...
  async function submit(path: string, formId: string, event: FormEvent<HTMLFormElement>, payload: Record<string, unknown>) {
    ...
    try { const message = await postForm(path, payload); setFeedback((current) => ({ ...current, [formId]: { type: "success", message } })); event.currentTarget.reset(); if (formId === "submission") setImageMetadata([]); }
